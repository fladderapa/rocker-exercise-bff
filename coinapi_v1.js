...
var COIN_API_SDK = (function () {
  ...
  COIN_API_SDK.prototype.metadata_list_exchanges = function () {
    ...
    return axios.get(path, { headers: this.headers, transformResponse: transformResponse }).then(function (resp) {
      return resp.data;
    });
  };
  COIN_API_SDK.prototype.metadata_list_assets = function (assetIds) {
    const path = assetIds === undefined ? `${this.url}/v1/assets` : `${this.url}/v1/assets?filter_asset_id=${assetIds}`;
    return axios.get(path, { headers: this.headers, transformResponse: transformResponse }).then(function (resp) {
      return resp.data;
    });
  };
  ...
  COIN_API_SDK.prototype.exchange_rates_get_specific_rate = function (asset_id_base, asset_id_quote, time) {
    ...
    return axios
      .get(path, { headers: this.headers, transformResponse: transformResponse, params: params })
      .then(function (resp) {
        return resp.data;
      });
  };
  ...
  COIN_API_SDK.prototype.ohlcv_historic_data = function (symbol_id, period_id, time_start, time_end, limit) {
    ...
    var path = this.url + ('/v1/ohlcv/' + symbol_id + '/history?period_id=' + period_id + '&time_start=' + time_start);
    var params = {};
    if (time_end) {
